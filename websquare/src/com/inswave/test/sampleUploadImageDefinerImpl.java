...
/**
 * websquare의 editor 컴포넌트 imageUpload에서 서버에 저장할 파일 패스, 파일명을 구현하는 Class 샘플.
 * websquare.upload.handl.AbstractUploadImageDefiner을 상속받아 getFileName, getFilePath를 구현한다.
 *
 */
public class sampleUploadImageDefinerImpl extends AbstractUploadImageDefiner {
	...
	public String getFileName( String clientFileName, String originalFileName ) throws Exception {
		...
		if(index > -1) {
			...
			returnFileName = clientFileName.substring(0, index) + "_IMAGE" + "." + ext;
		} else {
			returnFileName = clientFileName.substring(0) + "_IMAGE";
		}
    ...
    public String getFilePath( String filePath ) throws Exception {
		...
		String returnFilePath = "d:\\test\\customUploadImage";
